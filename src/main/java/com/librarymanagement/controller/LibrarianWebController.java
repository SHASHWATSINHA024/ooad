...
public class LibrarianWebController {
    ...
    public String dashboard(Model model, HttpSession session, RedirectAttributes redirectAttributes) {
        try {
            // Get current librarian from session
            UserDTO user = (UserDTO) session.getAttribute("currentUser");
            
            // Check if user is logged in and has librarian role
            if (user == null) {
                redirectAttributes.addFlashAttribute("error", "Please log in to access the librarian dashboard");
                return "redirect:/auth/login";
            }
            
            if (!"LIBRARIAN".equals(user.getRole()) && !"ADMIN".equals(user.getRole())) {
    ...
    public String approveUser(@PathVariable Long id, RedirectAttributes redirectAttributes, HttpSession session) {
        try {
            UserDTO sessionUser = (UserDTO) session.getAttribute("currentUser");
            if (sessionUser == null || !"LIBRARIAN".equals(sessionUser.getRole()) && !"ADMIN".equals(sessionUser.getRole())) {
    ...
    public String enableUser(@PathVariable Long id, RedirectAttributes redirectAttributes, HttpSession session) {
        try {
            UserDTO sessionUser = (UserDTO) session.getAttribute("currentUser");
            if (sessionUser == null || !"LIBRARIAN".equals(sessionUser.getRole()) && !"ADMIN".equals(sessionUser.getRole())) {
                redirectAttributes.addFlashAttribute("error", "Unauthorized access");
    ...
    public String disableUser(@PathVariable Long id, RedirectAttributes redirectAttributes, HttpSession session) {
        try {
            UserDTO sessionUser = (UserDTO) session.getAttribute("currentUser");
            if (sessionUser == null || !"LIBRARIAN".equals(sessionUser.getRole()) && !"ADMIN".equals(sessionUser.getRole())) {
                redirectAttributes.addFlashAttribute("error", "Unauthorized access");
    ...
    public String addBookForm(Model model, HttpSession session, RedirectAttributes redirectAttributes) {
        UserDTO sessionUser = (UserDTO) session.getAttribute("currentUser");
        if (sessionUser == null || !"LIBRARIAN".equals(sessionUser.getRole()) && !"ADMIN".equals(sessionUser.getRole())) {
            redirectAttributes.addFlashAttribute("error", "Unauthorized access");
            return "redirect:/auth/login";
        ...
        return "admin/book-form"; // Reuse the admin book form template
    }
    
    @PostMapping("/books/add")
    public String addBook(BookDTO book, RedirectAttributes redirectAttributes, HttpSession session) {
        try {
            UserDTO sessionUser = (UserDTO) session.getAttribute("currentUser");
            if (sessionUser == null || !"LIBRARIAN".equals(sessionUser.getRole()) && !"ADMIN".equals(sessionUser.getRole())) {
                redirectAttributes.addFlashAttribute("error", "Unauthorized access");
    ...
    public String editBookForm(@PathVariable Long id, Model model, RedirectAttributes redirectAttributes, HttpSession session) {
        UserDTO sessionUser = (UserDTO) session.getAttribute("currentUser");
        if (sessionUser == null || !"LIBRARIAN".equals(sessionUser.getRole()) && !"ADMIN".equals(sessionUser.getRole())) {
            redirectAttributes.addFlashAttribute("error", "Unauthorized access");
            return "redirect:/auth/login";
    ...
    public String updateBook(@PathVariable Long id, BookDTO book, RedirectAttributes redirectAttributes, HttpSession session) {
        try {
            UserDTO sessionUser = (UserDTO) session.getAttribute("currentUser");
            if (sessionUser == null || !"LIBRARIAN".equals(sessionUser.getRole()) && !"ADMIN".equals(sessionUser.getRole())) {
                redirectAttributes.addFlashAttribute("error", "Unauthorized access");
